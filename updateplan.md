...
| **D16** | Every single task — like, comment, follow, every scan — opened its own brand-new x.com tab and destroyed it right after, all day, every 8–45 seconds while the engine ran. Visibly a tab popping open and shut nonstop, not a calm colleague. Found post-build, by the owner watching it run. | `platforms/common/tab-driver.ts` (pre-fix): `chrome.tabs.create` in `driveTab`, `chrome.tabs.remove` in its `finally`. Called from ~20 sites in `executor.ts`, `background/index.ts`, `background/setup-read.ts`, `scheduler/dry-run.ts`, `scheduler/scheduled-posts.ts` — every one of them a fresh tab. |
| **D17** | The 14-day warm-up ramp started at a **10% floor** — so a brand-new setup got as few as 9–18 real actions on its first couple of days, on *any* preset, regardless of the account's own age. A real person doesn't ease into their normal posting pace over two weeks; this made the product look nearly inert during the exact days someone is deciding whether to keep using it. Found post-build, by the owner watching real daily counts (14–17/day observed against a 180/day Balanced base). | `lib/presets.ts` (pre-fix): `WARMUP_DAYS = 14`, `WARMUP_FLOOR = 0.1`, composed multiplicatively with `ageMultiplier` in `quotas.ts:44` (`m = ageMultiplier(months) * warmup`) — a young/unknown account (0.5×) on day 1 (~0.16×) yields **~8% of base caps**, matching the observed count almost exactly. |
| **D18** | An empty topics list made the home feed engage **every single post**, back to back — liked, bookmarked and replied to indiscriminately, with nothing to do with growing the right audience. This was a deliberate, documented default ("leave blank to engage with everything"), but it is mass engagement, not targeted growth, and reads as automated to X itself. Found post-build, by the owner watching it work through their feed. | `platforms/common/relevance.ts` (pre-fix): `isRelevant` returned `keywords.length === 0 \|\| matchesAny(...)` — an empty `keywords` array short-circuited to `true` for every post. |
| **D19** | Search feeds (the one engagement source that reaches people outside the accounts already followed) never ran for any user who completed Setup, because Setup never wrote anything to `settings.searchQueries` — it stayed `[]` until someone manually found "Who I watch → Topic feeds" and retyped the same topics by hand. In practice this meant a fresh install's only real diversity was whichever 2-3 target creators survived step 1, on repeat. Found post-build, by the owner noticing the engine "keeps working around 2-3 people." | `sidepanel/pages/Setup.tsx` (pre-fix): `finish()` wrote `contentTopics`, `homeFeed.keywords` and `targetCreators` but never `searchQueries`. |
...
| 2026-09-11 | **D16** One tab, reused | `cad8116` | Found by the owner watching it run, not by the original audit: `platforms/common/tab-driver.ts` created a brand-new x.com tab for every single task and destroyed it right after — a tab visibly opening and closing every 8–45 seconds, all day. Rewritten to keep **one** tab, created once, reused and re-navigated (`chrome.tabs.update`, not `create`) for every like/comment/follow/scan from then on. It is never made the active tab, so it never steals focus from whatever the user is doing elsewhere — they can switch to it any time to watch Spotlight live, but nothing forces them to. Its id lives in `chrome.storage.session` (not a module variable) because the service worker itself restarts mid-day; a discarded tab (Chrome's own memory-saver) is detected via `tab.discarded` and re-navigated even when its `url` already matches. Every call is serialized through a `withLock` queue so the independent mentions-scan alarm (4.1) can never navigate the shared tab out from under an in-flight action. No automated test: this repo has no chrome-API mocking harness for anything (same gap 6.4/6.8 already flagged for DOM code) — verify live, per the QA checklist. `pnpm --filter @casper/extension test` — 22 suites still green (behaviour untouched, only how the tab is acquired changed); typecheck and build clean. |
| 2026-09-11 | **D17** Warm-up ramp loosened | `16e5687` | Found by the owner comparing real daily counts (14–17/day) against what a real person does by hand (~100/day). Traced to `lib/presets.ts`: the 14-day ramp's 10% floor, multiplying with `ageMultiplier` in `quotas.ts`, crushed the very first days of ANY preset regardless of account age — a Balanced install's 180/day base became single digits on day one. `WARMUP_DAYS` **14 → 5**, `WARMUP_FLOOR` **0.1 → 0.4**: day one is now 40% of base before the age multiplier, full caps by day 5 instead of day 14 — still a real ramp (day one is not day five), just not a factor-of-ten gap between "safe" and "real" during the exact window someone decides whether to keep using the product. `ageMultiplier` itself (the permanent 0.5× for an account under 6 months, or of unknown age) is untouched — that reflects real platform risk on the ACCOUNT, not automation-newness, and stays user-controlled via the "under 6 months" checkbox from setup (1.4) / Settings. **Owner note:** if the test account is not actually under 6 months old, unchecking that box removes its own separate 0.5× — worth checking, since it compounds with the ramp. `scripts/presets-smoke.mts` updated to assert against the `WARMUP_FLOOR`/`WARMUP_DAYS` constants rather than the hardcoded 10%/55%/day-14 values the old ramp produced, so the test encodes the rule, not one ramp's numbers. All 22 suites green, typecheck and build clean. |
| 2026-09-11 | **D17 follow-up** Same-day caps, not next-midnight | `cefb485` | The ramp fix above didn't reach the UI: `effectiveCap` is computed once when today's counter is first created and then frozen in storage for the rest of the calendar day (by design, so the number doesn't jitter hour to hour) — so an install that already had a counter for today kept showing the OLD (pre-fix) number until local midnight, no matter how many times the extension was rebuilt. Two changes, together: (1) `quotas.ts`'s `dailyVarianceFactor` is now seeded from `date:platform` (a small deterministic hash) instead of `Math.random()` — same day, same variance, every time it's computed, which means (2) `counters.ts`'s `ensureToday` can safely **recompute** `effectiveCap` for a day that already has a counter (previously it just returned the existing one untouched), re-persisting it whenever it differs from what's stored — a preset switch or a ramp-math fix like this one now reaches the UI the moment it's next computed, not at midnight. `background/index.ts` gained `refreshTodaysCaps()`, called from all three boot paths (`onInstalled`, `onStartup`, initial SW boot) — since the UI reads counters straight from storage rather than through `ensureToday`'s return value, and reloading the unpacked extension is exactly a boot, this guarantees the corrected number is already sitting in storage the moment the side panel is reopened, whether or not the engine happens to be Active. `platformCapsForToday` gained a required `date` parameter (its one caller already had it in scope). All 22 suites green, typecheck and build clean. |
| 2026-09-11 | **2.3 extended** Spotlight now shows reading, not just acting | `5885c99` | Owner ask: "I want to always see the copilot working, not just for one or two actions." Root cause: Spotlight (2.3) only ever lit up for a post the engine actually acted on — and with a real keyword match rate, most scrolled posts are read and passed over, so the outline was dark for most of a session, making it look idle in between rather than continuously working. `floating/spotlight.ts` gained a `'reading'` `SpotlightAction` — deliberately **not coral** (a dashed muted-grey outline via a new `.ghostly247-spotlight-reading` modifier class, no pulse), so "just looking at this" reads calmer than "about to change something," which stays reserved for a real action. `autopilot.ts`'s per-post read-dwell (`await wait(readDwellMs(...))`, spent on every post that reaches it whether or not anything comes of it) is now wrapped in `withSpotlight(article, 'reading', ...)` — so the outline and the panel line move to whatever post is currently being considered, continuously, for the whole scroll session, not only the rare match. If a post goes on to be acted on, that action's own `withSpotlight` call (already existed) replaces the reading outline with the real coral one a moment later — no special-casing needed, `spotlightOn` already clears before re-marking. `SpotlightLine.tsx` styles the `reading` state in muted/surface tokens instead of coral, with no pulsing dot, so the panel line matches the page outline. No automated test (this module is DOM/chrome-dependent with no mocking harness, same as the rest of Spotlight) — verify live: scrolling should now show a moving muted outline on whatever post is being read, snapping to a solid pulsing coral one only when it actually likes/replies/follows. All 22 suites green, typecheck and build clean. |
| 2026-09-11 | **D18** Empty topics engaged everything, not nothing | *(pending commit)* | Owner report: "I see it liking all the posts in a row on my feed, every single post — this is not based on growing the user, it just does it with all the posts." Traced to `relevance.ts`'s `isRelevant`: **"no keywords = engage with everything" was a deliberate, documented default** (surfaced in `WhoIWatch.tsx` as "Leave blank to engage with everything in your feed") — but in practice it meant every single post in an unfiltered feed got liked/bookmarked/replied to back to back, which is mass engagement, not targeted growth, and is exactly the pattern most likely to read as automated to X itself. Flipped: empty keywords now mean the home feed engages **nothing** (`keywords.length > 0 && matchesAny(...)`, was `=== 0 ||`) — a watched target's own post still bypasses this check entirely via `autopilot.ts`'s separate `authorIsWatched` relaxation, so following specific creators (itself a form of targeting) is unaffected. An empty-topics home feed now correctly surfaces as the existing `nothing-matched` card ("skipped N posts — nothing matched your topics — widen my topics") instead of silently acting on everything — no new block-reason state needed, this is precisely what that card was already built to say. `WhoIWatch.tsx`'s copy corrected to match. `scripts/relevance-smoke.mts` updated (the old assertion encoded the defect, same pattern as D3's fix). All 22 suites green, typecheck and build clean. |
| 2026-09-11 | **D19** Search feeds never ran because Setup never created any | *(pending commit)* | Owner report: "it is not able to search field related content and then engage with that — instead it keeps working around 2-3 people." Traced to `Setup.tsx`'s `finish()`: it writes `contentTopics` and `homeFeed.keywords` from the extracted topics, and target creators from the account read, but **never touches `settings.searchQueries`** — which defaults to `[]` and stays there until a user manually finds "Who I watch → Topic feeds" and retypes the same topics. Search feeds are the one source that reaches people OUTSIDE the accounts already followed (`SEARCH_INTERVAL_MS`, its own budget share, all built in 6.7) — leaving it empty meant a fresh setup's only real diversity was whatever handful of target creators survived step 1, which is exactly "2-3 people" on repeat. Two changes: (1) `Setup.tsx` now seeds `searchQueries` from the same kept topics used for `homeFeed.keywords`, for every **new** setup from here on. (2) For the install already past setup (this doesn't reach retroactively), `WhoIWatch.tsx`'s Topic feeds section gained a one-click **"+ Add my N topics as search feeds too"** button — reads `settings.contentTopics`, adds whichever aren't already a saved search, capped at `MAX_SEARCH_QUERIES`. Deliberately a visible, user-initiated action rather than a silent background migration: a migration that re-populates the list every time it reads empty would fight a user who deliberately cleared it. All 22 suites green, typecheck and build clean. |
| 2026-09-11 | **Copy** "Auto-approve replies" made unmistakable | *(pending commit)* | Owner ask: "it always asking me for the replies to approve — add auto approve so no manual work is needed." The capability already existed (`settings.replyApproval`, a checkbox in Voice — unchecking it makes replies post immediately, verified true for every reply path: home feed, mentions, and search all read the same flag) — the ask was purely that nobody could tell that from the copy. `Voice.tsx`'s section renamed "Review before posting" → **"Auto-approve replies"**, checkbox re-worded to "Post replies automatically — don't ask me first" (inverted so checking the box is the affirmative "yes, auto-approve" action instead of unchecking a box titled the opposite of what you want), and the helper line under it spells out that turning it on means nothing left to click. No behavioural change — this was a wording bug, not a logic bug. |
