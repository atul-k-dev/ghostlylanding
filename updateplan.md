...
> **Status:** Phase 0 in progress (0.1–0.3, 0.6 done · 2.5 spike awaiting Chrome verification)
...
- [ ] **0.7 — Correct the scheduler header comment.**
...
| 2026-09-10 | **0.3** Unknown age | `_(this commit)_` | `ageMultiplier(null)` returns **0.5**, not 1.0, and negatives with it; comment rewritten to say why (the field is optional, so unknown was the common case and it granted full caps to the youngest accounts). `Dashboard.tsx:769`'s "Leave blank if unsure" now reads "Left blank, I assume new and go at half pace" — the copy was half of D3. **D3 closed.** |
| 2026-09-10 | **0.3 test** | `_(this commit)_` | `scheduler-smoke.mts:56` asserted `ageMultiplier(null) === 1.0` — that assertion encoded the defect, so it now expects 0.5, with a note saying so, plus a `computeDailyCaps(base, null)` case. `pacing-smoke.mts` +6 assertions (the four the plan lists, plus negative-age and unknown-equals-brand-new). 8 suites green, typecheck clean. |
